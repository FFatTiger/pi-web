...
export const ThinkingContentSchema = z.strictObject({
 ...
 */
export const ToolCallContentSchema = z.strictObject({
...
export const TokenUsageCostSchema = z.strictObject({
  input: z.number(),
  output: z.number(),
  cacheRead: z.number(),
  cacheWrite: z.number(),
...
export const CustomMessageSchema = z.strictObject({
...
export const BashExecutionMessageSchema = z.strictObject({
...
/** Complete normalized agent message (message_end / history). */
...
/**
 * Streaming message DTOs for message_start / message_update.
 * Content keeps role-correct block/string/array types — never null/number/arbitrary objects.
 * Required fields that are not yet known may be omitted (partial), but present values are typed.
 */
export const StreamingUserMessageSchema = z.strictObject({
  role: z.literal("user"),
  content: UserContentSchema.optional(),
  timestamp: z.number().optional(),
});

export const StreamingAssistantMessageSchema = z.strictObject({
  role: z.literal("assistant"),
...
export const StreamingToolResultMessageSchema = z.strictObject({
  ...
  toolName: z.string().optional(),
  content: z.array(z.union([TextContentSchema, ImageContentSchema])).optional(),
  isError: z.boolean().optional(),
  details: z.unknown().optional(),
  timestamp: z.number().optional(),
});

export const StreamingCustomMessageSchema = z.strictObject({
  role: z.literal("custom"),
  customType: NonEmptyStringSchema.optional(),
  content: UserContentSchema.optional(),
...
export const StreamingBashExecutionMessageSchema = z.strictObject({
  role: z.literal("bashExecution"),
...
export const StreamingAgentMessageSchema = z.discriminatedUnion("role", [
  StreamingUserMessageSchema,
  StreamingAssistantMessageSchema,
  StreamingToolResultMessageSchema,
  StreamingCustomMessageSchema,
  StreamingBashExecutionMessageSchema,
]);
