...
**Upstream baseline:** this fork tracks upstream through **v0.8.7 / Pi 0.84.0** (ported selectively onto the Codex multi-project shell), including Node.js >=22.19, loopback-default bind with explicit LAN scripts, PWA support, i18n, model discovery/catalog, project trust, request/path security and upload body limits, session-title/path hardening, model runtime reload/errors, markdown images/Mermaid/LaTeX, slash-command collapse, turn-written file chips, session active time, factory extension widgets, selected-file-line and whole-file mentions, ChatInput history, minimap, and related security fixes. Fork-only production features above remain (password gate, multi-project UX, mobile polish). Compare [`agegr/pi-web`](https://github.com/agegr/pi-web) before the next rebase or release.
...
Behind HTTPS tunnels (frp, nginx, Caddy) the browser Origin is often `https://…` while Next sees `http://…`. API host checks still require the public hostname in `PI_WEB_ALLOWED_HOSTS` (comma-separated exact hosts). Origin checks then allow the same host+port with either scheme, and may honor the first `X-Forwarded-Proto` hop for non-loopback Host values (proxies should overwrite client-supplied values). For `next dev` only, the same hosts are also fed into Next `allowedDevOrigins`; optional globs can go in `PI_WEB_ALLOWED_DEV_ORIGINS` (e.g. `*.tunnel.example.com`).
