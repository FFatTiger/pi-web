...
test("allows HTTPS tunnel Origin when Next only sees the HTTP origin", async () => {
  const { isApiRequestAllowed } = await loadSubject();
  const hosts = ["pi-web.example.com"];
  const viaHttpsOrigin = new Request("http://127.0.0.1:30141/api/gate/login", {
    method: "POST",
    headers: {
      host: "pi-web.example.com",
      origin: "https://pi-web.example.com",
      "sec-fetch-site": "same-origin",
      "content-type": "application/json",
    },
  });
  const viaForwardedProto = new Request("http://127.0.0.1:30141/api/gate/login", {
    method: "POST",
    headers: {
      host: "pi-web.example.com",
      origin: "https://pi-web.example.com",
  ...
  const viaSameSiteSchemeMismatch = new Request("http://127.0.0.1:30141/api/gate/login", {
    method: "POST",
    headers: {
      host: "pi-web.example.com",
      origin: "https://pi-web.example.com",
  ...
  const foreignHttpsOrigin = new Request("http://127.0.0.1:30141/api/gate/login", {
    method: "POST",
    headers: {
      host: "pi-web.example.com",
      origin: "https://attacker.example",
  ...
  assert.equal(isApiRequestAllowed(viaHttpsOrigin, hosts), true);
  assert.equal(isApiRequestAllowed(viaForwardedProto, hosts), true);
  assert.equal(isApiRequestAllowed(viaSameSiteSchemeMismatch, hosts), true);
  assert.equal(isApiRequestAllowed(foreignHttpsOrigin, hosts), false);
});

test("rejects HTTPS tunnel Origin when host or port does not match", async () => {
  const { isApiRequestAllowed } = await loadSubject();
  const hosts = ["pi-web.example.com"];
  const originPortMismatch = new Request("http://127.0.0.1:30141/api/gate/login", {
    method: "POST",
    headers: {
      host: "pi-web.example.com",
      origin: "https://pi-web.example.com:9999",
      "sec-fetch-site": "same-origin",
      "content-type": "application/json",
    },
  });
  const hostPortMismatch = new Request("http://127.0.0.1:30141/api/gate/login", {
    method: "POST",
    headers: {
      host: "pi-web.example.com:30141",
      origin: "https://pi-web.example.com",
      "sec-fetch-site": "same-origin",
      "content-type": "application/json",
    },
  });
  const matchingNonDefaultPort = new Request("http://127.0.0.1:30141/api/gate/login", {
    method: "POST",
    headers: {
      host: "pi-web.example.com:8443",
      origin: "https://pi-web.example.com:8443",
      "sec-fetch-site": "same-origin",
      "content-type": "application/json",
    },
  });

  assert.equal(isApiRequestAllowed(originPortMismatch, hosts), false);
  assert.equal(isApiRequestAllowed(hostPortMismatch, hosts), false);
  assert.equal(isApiRequestAllowed(matchingNonDefaultPort, hosts), true);
});

test("ignores X-Forwarded-Proto for loopback Host values", async () => {
  const { isApiRequestAllowed } = await loadSubject();
  // Spoofed XFP must not turn a loopback Host into an https request origin match
  // against a foreign https Origin; cross-site still fails first, and host mismatch
  // with loopback remains rejected.
  const spoofedLoopback = new Request("http://127.0.0.1:30141/api/test", {
    method: "POST",
    headers: {
      host: "localhost:30141",
      origin: "https://attacker.example",
  ...
  assert.equal(isApiRequestAllowed(spoofedLoopback), false);
});
