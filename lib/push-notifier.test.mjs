...
test("invalid session ids never reach foreground presence or Web Push", async () => {
  for (const sessionId of ["", "s".repeat(257)]) {
    const calls = [];
    const notifier = new mod.PushNotifier({
      readGateConfig: () => ({ status: "enabled", configPath: "/tmp/pi-web.json", password: "secret" }),
      readPushConfig: () => ({ status: "enabled", configPath: "/tmp/pi-web.json", subject: "mailto:owner@example.com" }),
    ...
    await notifier.handleSettled({
      sessionId,
      cycleId: 1,
      messages: [{ role: "assistant", stopReason: "stop" }],
    });

    assert.deepEqual(calls, [], JSON.stringify(sessionId));
  }
});
