...
test("rejects empty endpoint without writing invalid state or locking the store", async () => {
  const { store, statePath } = makeStore();
  await store.getVapidKeys();
  const before = readFileSync(statePath, "utf8");
  await assert.rejects(
    store.upsert({ endpoint: "", p256dh: "k", auth: "a" }, "secret"),
    (error) => error instanceof TypeError && !String(error).includes("secret") && !String(error).includes(statePath),
  );
  assert.equal(readFileSync(statePath, "utf8"), before);
  assert.equal(await store.upsert({ endpoint: "https://push.example/a", p256dh: "k", auth: "a" }, "secret"), "created");
  assert.ok(await store.findAuthorized("https://push.example/a", "secret"));
});

test("rejects empty p256dh without writing invalid state or locking the store", async () => {
  const { store, statePath } = makeStore();
  await store.getVapidKeys();
  const before = readFileSync(statePath, "utf8");
  await assert.rejects(
    store.upsert({ endpoint: "https://push.example/a", p256dh: "", auth: "a" }, "secret"),
    (error) => error instanceof TypeError && !String(error).includes("https://push.example"),
  );
  assert.equal(readFileSync(statePath, "utf8"), before);
  assert.equal(await store.upsert({ endpoint: "https://push.example/a", p256dh: "k", auth: "a" }, "secret"), "created");
});

test("rejects empty auth without writing invalid state or locking the store", async () => {
  const { store, statePath } = makeStore();
  await store.getVapidKeys();
  const before = readFileSync(statePath, "utf8");
  await assert.rejects(
    store.upsert({ endpoint: "https://push.example/a", p256dh: "k", auth: "" }, "secret"),
    (error) => error instanceof TypeError,
  );
  assert.equal(readFileSync(statePath, "utf8"), before);
  assert.equal(await store.upsert({ endpoint: "https://push.example/b", p256dh: "k", auth: "a" }, "secret"), "created");
  const store2 = new mod.PushStore({
    statePath,
    generateVapidKeys: () => ({ publicKey: "x", privateKey: "y" }),
  });
  assert.equal(await store2.getPublicKey(), "public-key");
  assert.ok(await store2.findAuthorized("https://push.example/b", "secret"));
});

test("rejects empty generated VAPID keys without leaving invalid state file", async () => {
  const { statePath, dir } = makeStore();
  const store = new mod.PushStore({
    statePath,
    generateVapidKeys: () => ({ publicKey: "", privateKey: "private-key" }),
  });
  await assert.rejects(store.getVapidKeys(), (error) => {
    const text = String(error) + (error && error.stack ? error.stack : "");
    return error instanceof TypeError
      && !text.includes("private-key")
      && !text.includes(statePath);
  });
  assert.equal(readdirSync(dir).includes("pi-web-push.json"), false);
  assert.deepEqual(readdirSync(dir).filter((name) => name.includes(".tmp-")), []);

  const store2 = new mod.PushStore({
    statePath,
    generateVapidKeys: () => ({ publicKey: "public-key", privateKey: "" }),
  });
  await assert.rejects(store2.getVapidKeys(), (error) => error instanceof TypeError);
  assert.equal(readdirSync(dir).includes("pi-web-push.json"), false);
});

test("rejects unknown own keys on load without overwrite", async () => {
  const { statePath } = makeStore();
  const withExtraTop = JSON.stringify({
    version: 1,
    vapid: { publicKey: "public-key", privateKey: "private-key" },
    subscriptions: [],
    extra: true,
  });
  writeFileSync(statePath, withExtraTop, { mode: 0o600 });
  const store = new mod.PushStore({ statePath });
  await assert.rejects(store.getPublicKey(), (error) => error.code === "PUSH_STORE_LOCKED");
  assert.equal(readFileSync(statePath, "utf8"), withExtraTop);

  const withExtraVapid = JSON.stringify({
    version: 1,
    vapid: { publicKey: "public-key", privateKey: "private-key", kid: "x" },
    subscriptions: [],
  });
  writeFileSync(statePath, withExtraVapid, { mode: 0o600 });
  const store2 = new mod.PushStore({ statePath });
  await assert.rejects(store2.getVapidKeys(), (error) => error.code === "PUSH_STORE_LOCKED");
  assert.equal(readFileSync(statePath, "utf8"), withExtraVapid);

  const withExtraSub = JSON.stringify({
    version: 1,
    vapid: { publicKey: "public-key", privateKey: "private-key" },
    subscriptions: [{
      endpoint: "https://push.example/a",
  ...
  writeFileSync(statePath, withExtraSub, { mode: 0o600 });
  const store3 = new mod.PushStore({ statePath });
  await assert.rejects(store3.getPublicKey(), (error) => error.code === "PUSH_STORE_LOCKED");
  assert.equal(readFileSync(statePath, "utf8"), withExtraSub);
});
