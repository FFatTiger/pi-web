...
- `kind` 只允许 `agent` 或 `test`。
- `agent.result` 只允许 `success` 或 `error`。
- Service Worker 和 AppShell 使用固定映射生成标题、正文和 tag。
- `agent` payload 从 `sessionId` 自行构造 `/?session=${encodeURIComponent(sessionId)}`；`test` payload 固定打开 `/`。
...
- test：`Test notification delivered`
...
测试接口只允许向当前认证指纹下已登记的 endpoint 发送固定 `{version: 1, id, kind: "test"}` payload。客户端必须提交当前 endpoint，服务端不能接受任意目标 URL、sessionId 或任意通知正文。
...
- 解析固定的 version 1 判别式 payload：agent 为 `{version,id,kind:"agent",sessionId,result}`，test 为 `{version,id,kind:"test"}`。
- 拒绝未知版本、未知 kind、缺少 id、agent 缺少 sessionId 或未知 result；不能接受服务端任意标题、正文、tag 或 URL。
- 使用本地固定映射生成 Pi Web 标题、正文、icon、badge 和 tag。
- agent 从 `sessionId` 自行构造站内相对链接；test 固定使用 `/`。
...
3. agent payload 从已校验的 `sessionId` 构造 `/?session=<encoded-id>`；test payload 固定使用 `/`，不读取 payload 中的任意 URL。
...
- notification click 对 agent payload 从受控 sessionId 构造同 origin URL，对 test payload 固定使用 `/`，两者都不接受 payload 任意 URL。
...
- 发送异常不影响 Agent 流程。
- versioned payload 使用判别式固定字段，不含任意文案、URL 或敏感字段。
